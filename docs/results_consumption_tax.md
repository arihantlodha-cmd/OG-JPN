...
Real Japan demographics (UN, code 392); percent change in the steady-state aggregate vs the 10% baseline. Macro/tax parameters are first-pass (see docs/NEXT_STEPS.md), so read the signs and rough magnitudes, not the last digit. Baseline pension outlays are 11.4% of GDP.

| Reform | Y (%) | C (%) | K (%) | L (%) | Revenue/GDP |
|---|---|---|---|---|---|
| 10% -> 12% | +0.04 | -1.65 | -0.25 | +0.27 | 0.322 -> 0.336 |
| 10% -> 15% | +0.11 | -4.03 | -0.61 | +0.66 | 0.322 -> 0.355 |
